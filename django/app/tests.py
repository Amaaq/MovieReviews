...
class MovieViewSetTestCase(TestCase):
    def setUp(self):
        ...
        self.movie1 = Movie.objects.create(
            title="Test Movie 1", description="Test Description 1"
        )
        self.movie2 = Movie.objects.create(
            title="Test Movie 2", description="Test Description 2"
        )
    ...
    def test_movie_update(self):
        response = self.client.patch(
            "/movies/2/", payload={"description": "Updated Description", "actors": [1]}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
